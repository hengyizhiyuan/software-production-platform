...
class CodexSdkWorkInteractionCapability:
    ...
    def _instruction(basis: InteractionInterpretationInput) -> str:
        ...
        return (
            ...
            "natural_response, use progressive disclosure: do not enumerate the full "
            "Design Schema, methodology, agenda, or every unresolved issue. Keep the "
            "response concise and conversational. Acknowledge and use facts the Human "
            "already supplied; briefly explain the relevant design approach and current "
            "stage; choose the highest-value current focus and explain why it comes next; "
            "proactively offer a concrete framing, direction, alternatives, trade-offs, "
            "or decision order when the evidence supports them; recommend one next design "
            "action; and ask at most one highest-impact unresolved question. Treat every "
            "fact in the persisted records as known Conversation Reality and never ask "
            "the Human to repeat it. Do not turn the response into a questionnaire or "
            "reduce it to questions: lead with a useful proposal whenever the evidence "
            "permits. Explain progress without claiming that advisory interpretation "
