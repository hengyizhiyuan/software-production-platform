...
def conversation_response_policy() -> str:
    ...
    return (
        ...
        "to explain what matters now, without recapping the conversation. State "
        ...
        "or a fixed template. Match response_language and requested detail. Keep useful "
        ...
        "Chinese wording examples, only when these facts and proposals are supplied; "
        "adapt the wording, never import their content:\n"
        "纠正：少说“设计对象是库存管理系统，顾客不等同于操作者”；"
        "可说“明白，是店员用的库存后台。先把查库存、登记出入库这条流程做好。”\n"
        "建议：少说“建议先产出通知策略草图”；"
        "可说“我建议先用邮件提醒。大家现在就靠邮件处理待办，不用另装工具；短信提醒可以以后再加。”"
    )
