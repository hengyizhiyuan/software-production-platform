(() => {
  ...
  function showDelivery(entry) {
    ...
    const download=node("a","下载文档包与验证清单","download");download.href=`/api/works/${work.work_id}/deliveries/${manifest.id}/download`;card.append(download);
    if(decision)card.append(node("p",decision.authority_identity+"："+decision.rationale));
    if(entry.current&&!decision){const form=node("form"),label=node("label"),checked=document.createElement("input");checked.type="checkbox";checked.required=true;label.append(checked,document.createTextNode("我已查看这个版本的文件和验收标准"));
  ...
  async function refresh(){const currentGeneration=++generation;const works=await api("/api/works");const list=Array.isArray(works)?works:works.works||[];if(currentGeneration!==generation)return;
    ...
    work=current;$("assets").replaceChildren();assets.forEach(showAsset);$("work-summary").textContent=work?`${work.desired_outcome} · ${work.status} · ${work.what_happens_next}`:"先接入仓库，或返回对话澄清并准入一个尚无仓库的 Work。";
    ...
    $("deliveries").replaceChildren();$("target-summary").textContent=delivery&&delivery.target?`${delivery.target.title}：${delivery.target.acceptance_criteria.join("；")}`:"尚未记录交付目标。";$("target").hidden=Boolean(delivery&&delivery.target);if(delivery)delivery.deliveries.forEach(showDelivery);
  }
