(() => {
  ...
  async function refresh(){const currentGeneration=++generation;const works=await api("/api/works");const list=Array.isArray(works)?works:works.works||[];if(currentGeneration!==generation)return;
    ...
    $("deliveries").replaceChildren();$("target-summary").textContent=delivery&&delivery.target?`${delivery.target.title}：${delivery.target.acceptance_criteria.join("；")}`:"尚未记录交付目标。";$("target").hidden=Boolean(delivery&&delivery.target);if(delivery){await Promise.all(delivery.deliveries.filter(entry=>entry.manifest.software).map(async entry=>{entry.runtime=await api(`/api/works/${selected}/deliveries/${entry.manifest.id}/runtime`);}));if(currentGeneration!==generation)return;delivery.deliveries.forEach(showDelivery);}
  }
  ...
  $("target").addEventListener("submit",event=>{event.preventDefault();action(()=>api(`/api/works/${requireWork()}/delivery-target`,{kind:$("target-kind").value,title:$("target-title").value,acceptance_criteria:$("criteria").value.split("\n").map(s=>s.trim()).filter(Boolean),authority_identity:"human:local-operator",...($("target-kind").value==="SOFTWARE_ARTIFACT"?{software_form:$("software-form").value,runtime_recipe:{adapter:"STATIC_WEB",entrypoint:$("entrypoint").value.trim()}}:{})}));});
