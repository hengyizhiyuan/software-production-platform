...
**FORMAL UX/UI APPEARANCE ARCHITECTURE — DOCUMENTED / IMPLEMENTATION NOT STARTED.** Watt now distinguishes two bounded appearance scopes: `GLOBAL_LIGHT` / `GLOBAL_DARK` govern the product shell and ordinary pages, while six registered Workspace Skins govern only the central Work experience. Visual evolution must remain cheap and behavior-preserving; the approved four-surface Workspace semantics do not vary by skin. All six exact Human-approved visual references are preserved under stable canonical asset names. See [Watt Formal UX/UI — Appearance Architecture & Workspace Skin Library](docs/architecture/watt-formal-ui-appearance-and-skin-library.md).
