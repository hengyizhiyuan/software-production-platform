...
from benchmarks.native_executor_continuity_v4.evaluator import evaluate
from benchmarks.native_executor_continuity_v4.runner import (
    BudgetLedger,
    SPEC_PATH,
    plan,
...
EXPECTED_DIGEST = "04affa90cf744deef9db257156014910ea712bad9aea28d3634e7de45f784124"
...
def test_v4_plan_preserves_the_frozen_execution_count_and_replacements() -> None:
    spec = _spec()
    generated = plan(spec)
    frozen = json.loads(
        (SPEC_PATH.parent / "../../.spg/validation-evidence/native-cont-v4/frozen-plan.json")
        .resolve()
        .read_text()
    )
...
def test_v4_budget_includes_the_original_conservative_spend(tmp_path: Path) -> None:
    spec = _spec()
    ledger = BudgetLedger(tmp_path / "results.json", spec)

    assert ledger.data["prior_conservative_cost_rmb"] == 3.88252724
    assert spec["hard_cost_limit"] == 100.0


def test_v4_t5_verification_isolated_by_repository() -> None:
...
def test_v4_evaluator_applies_frozen_scope_and_quality_anchors(tmp_path: Path) -> None:
