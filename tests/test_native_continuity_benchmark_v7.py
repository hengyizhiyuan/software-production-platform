...
from benchmarks.native_executor_continuity_v7.evaluator import evaluate
from benchmarks.native_executor_continuity_v7.runner import (
    BudgetLedger,
    SPEC_PATH,
    plan,
...
EXPECTED_DIGEST = "3a5bc92249a10f7ee646ab832ff4012d4b9fc29f4258e6fb9ea60db8d9ecb9ac"
...
def test_v7_plan_preserves_the_frozen_execution_count_and_replacements() -> None:
    spec = _spec()
    generated = plan(spec)
    frozen = json.loads(
        (SPEC_PATH.parent / "../../.spg/validation-evidence/native-cont-v7/frozen-plan.json")
        .resolve()
        .read_text()
    )
...
def test_v7_budget_includes_the_original_conservative_spend(tmp_path: Path) -> None:
    spec = _spec()
    ledger = BudgetLedger(tmp_path / "results.json", spec)

    assert ledger.data["prior_conservative_cost_rmb"] == 5.02900794
    assert spec["hard_cost_limit"] == 100.0


def test_v7_t5_verification_isolated_by_repository() -> None:
...
def test_v7_evaluator_applies_frozen_scope_and_quality_anchors(tmp_path: Path) -> None:
