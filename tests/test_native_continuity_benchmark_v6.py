...
from benchmarks.native_executor_continuity_v6.evaluator import evaluate
from benchmarks.native_executor_continuity_v6.runner import (
    BudgetLedger,
    SPEC_PATH,
    plan,
...
EXPECTED_DIGEST = "728300efe599788491ccea81d3f9bdcf66d212a83664d02f2e4131b691ea5656"
...
def test_v6_plan_preserves_the_frozen_execution_count_and_replacements() -> None:
    spec = _spec()
    generated = plan(spec)
    frozen = json.loads(
        (SPEC_PATH.parent / "../../.spg/validation-evidence/native-cont-v6/frozen-plan.json")
        .resolve()
        .read_text()
    )
...
def test_v6_budget_includes_the_original_conservative_spend(tmp_path: Path) -> None:
    spec = _spec()
    ledger = BudgetLedger(tmp_path / "results.json", spec)

    assert ledger.data["prior_conservative_cost_rmb"] == 4.67781665
    assert spec["hard_cost_limit"] == 100.0


def test_v6_t5_verification_isolated_by_repository() -> None:
...
def test_v6_evaluator_applies_frozen_scope_and_quality_anchors(tmp_path: Path) -> None:
