...
def test_settings_load_from_environment(monkeypatch) -> None:
    ...
    monkeypatch.setenv("SPG_WIC_PROVIDER_MODEL", "deepseek-flash")
    monkeypatch.setenv("SPG_WIC_PROVIDER_ADAPTER", "deepseek")
    monkeypatch.setenv("SPG_CONVERSATION_PROVIDER_ADAPTER", "deepseek")
    monkeypatch.setenv("SPG_CONVERSATION_PROVIDER_MODEL", "deepseek-flash")
    ...
    assert settings.wic_provider_adapter == "deepseek"
    assert settings.wic_provider_model == "deepseek-flash"
    assert settings.conversation_provider_adapter == "deepseek"
    assert settings.conversation_provider_model == "deepseek-flash"
...
def test_collaboration_provider_settings_are_independent_of_executor(monkeypatch) -> None:
    ...
    settings = Settings(wic_provider_adapter="deepseek", executor_adapter="unconfigured")
    assert settings.executor_timeout_seconds == 600
    assert settings.wic_coalesce_pre_work is True
    with patch("spg.providers.deepseek_interaction.DeepSeekWorkInteractionCapability") as provider:
        ...
        try:
            ...
            assert runtime.profile(ModelPurpose.WIC_SEMANTIC).timeout_seconds == 45
            assert runtime.profile(ModelPurpose.WIC_SEMANTIC).reasoning_effort == "medium"
            assert runtime.profile(ModelPurpose.CONVERSATION_RESPONSE).reasoning_effort == "low"
        finally:
            service.shutdown()
