...
from benchmarks.native_executor_continuity_v3.evaluator import evaluate
from benchmarks.native_executor_continuity_v3.runner import (
    BudgetLedger,
    SPEC_PATH,
    plan,
...
EXPECTED_DIGEST = "3877434d4c25e80dda179133054495e80b3be26ff0488b9f502f71ad6004d056"
...
def test_v3_plan_preserves_the_frozen_execution_count_and_replacements() -> None:
    spec = _spec()
    generated = plan(spec)
    frozen = json.loads(
        (SPEC_PATH.parent / "../../.spg/validation-evidence/native-cont-v3/frozen-plan.json")
        .resolve()
        .read_text()
    )
...
def test_v3_budget_includes_the_original_conservative_spend(tmp_path: Path) -> None:
    spec = _spec()
    ledger = BudgetLedger(tmp_path / "results.json", spec)

    assert ledger.data["prior_conservative_cost_rmb"] == 3.74041178
    assert spec["hard_cost_limit"] == 100.0


def test_v3_t5_verification_isolated_by_repository() -> None:
...
def test_v3_evaluator_applies_frozen_scope_and_quality_anchors(tmp_path: Path) -> None:
