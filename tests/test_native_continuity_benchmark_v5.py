...
from benchmarks.native_executor_continuity_v5.evaluator import evaluate
from benchmarks.native_executor_continuity_v5.runner import (
    BudgetLedger,
    SPEC_PATH,
    plan,
...
EXPECTED_DIGEST = "e41cb81112581bf5bf252d923c28a668d1e57c4af839676b36bf733a50cee74c"
...
def test_v5_plan_preserves_the_frozen_execution_count_and_replacements() -> None:
    spec = _spec()
    generated = plan(spec)
    frozen = json.loads(
        (SPEC_PATH.parent / "../../.spg/validation-evidence/native-cont-v5/frozen-plan.json")
        .resolve()
        .read_text()
    )
...
def test_v5_budget_includes_the_original_conservative_spend(tmp_path: Path) -> None:
    spec = _spec()
    ledger = BudgetLedger(tmp_path / "results.json", spec)

    assert ledger.data["prior_conservative_cost_rmb"] == 3.99016268
    assert spec["hard_cost_limit"] == 100.0


def test_v5_t5_verification_isolated_by_repository() -> None:
...
def test_v5_evaluator_applies_frozen_scope_and_quality_anchors(tmp_path: Path) -> None:
