...
from spg.application.delivery import candidate_file_inventory
...
def test_document_candidate_uses_only_exact_artifact_in_large_repository(tmp_path: Path) -> None:
    ...
    with pytest.raises(ProductInvariantViolation, match="bounded file inventory"):
        candidate_file_inventory(str(repo), revision, ("index.html",))
