...
class BudgetLedger:
    ...
    def before_request(self, model: str, request) -> str:
        ...
        if self.data["actual_cost_rmb"] + self.data["reserved_uncertain_cost_rmb"] + reserve > self.spec["hard_cost_limit"]:
            self.flush()
...
def admission_for(runtime_service: RuntimeService, task: dict, root: Path, repos: dict[str, Path], model: str, permitted: tuple[str, ...], *, prior_attempt=None, attempt_override=None, session_id=None, revision=1, contract_additions: dict | None = None):
    ...
    completion = CompletionContract(
        ...
        verification_obligations=tuple(task["verification"]),
    )
    ...
    workspace = WorkspaceManifest(workspace_id=uuid4(), work_id=work_id, pwu_id=pwu_id, attempt_id=attempt.id, source_vector_digest=vector.digest or "", host_storage_id=str(root), environment_profile_digest=canonical_digest({"profile":"continuity-v1"}), mounts=tuple(mounts), evidence_namespace=f"continuity:{attempt.id}", retention_policy="benchmark-retained")
    ...
    envelope = ResourceEnvelope(envelope_id=uuid4(), policy_version="continuity-v1", max_inference_submissions=30, max_tool_effects=100, max_active_seconds=1800, max_cost_units=10000, provider_profile=exact_profile(model), permitted_provider_profiles=permitted)
    admitted_paths = [
        workspace_path(mount_id, path)
        for mount_id, repo in task["repositories"].items()
        for path in repo["write_scope"]
...
def validate(root: Path, task: dict) -> list[dict]:
    ...
    for command in task["verification"]:
        if command.startswith("preview.inspect "):
            path=root/command.split(" ",1)[1]
            results.append({"command":command,"returncode":0 if path.is_file() else 1})
            continue
        argv=shlex.split(command)
        if argv and argv[0] == "pytest":
            argv=[sys.executable,"-m","pytest",*argv[1:]]
        proc=subprocess.run(argv,cwd=root,env=safe_env,capture_output=True,text=True,timeout=180)
...
async def run_trial(spec: dict, trial: dict, settings: Settings, ledger: BudgetLedger, database) -> dict:
    ...
    if progress["attempts"]:
        ...
        root,repos=prepare_repositories(settings.native_executor_workspace_root/"continuity"/trial["execution_id"],task)
...
def enrich_completed_trial_evidence(ledger: BudgetLedger, database) -> None:
    ...
    for trial in ledger.data["trials"]:
        ...
        trial["engineering_quality_score"]={
            "status":"UNSCORED_CONTRACT_GAP",
            "score":None,
            "reason":"frozen spec defines five 0-4 category maxima but no scoring anchors",
        }
    ledger.flush()
