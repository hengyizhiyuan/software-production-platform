...
def admission_for(runtime_service: RuntimeService, task: dict, root: Path, repos: dict[str, Path], model: str, permitted: tuple[str, ...], *, prior_attempt=None, attempt_override=None, session_id=None, revision=1, contract_additions: dict | None = None):
    ...
    workspace = WorkspaceManifest(workspace_id=uuid4(), work_id=work_id, pwu_id=pwu_id, attempt_id=attempt.id, source_vector_digest=vector.digest or "", host_storage_id=str(root), environment_profile_digest=canonical_digest({"profile":"continuity-v6"}), mounts=tuple(mounts), evidence_namespace=f"continuity-v6:{attempt.id}", retention_policy="benchmark-retained")
    ...
    envelope = ResourceEnvelope(envelope_id=uuid4(), policy_version="continuity-v6", max_inference_submissions=30, max_tool_effects=100, max_active_seconds=1800, max_cost_units=10000, provider_profile=exact_profile(model), permitted_provider_profiles=permitted)
    admitted_paths = [
        workspace_path(mount_id, path)
        for mount_id, repo in task["repositories"].items()
        for path in repo["write_scope"]
...
async def run_trial(spec: dict, trial: dict, settings: Settings, ledger: BudgetLedger, database) -> dict:
    ...
    if progress["attempts"]:
        ...
        root,repos=prepare_repositories(settings.native_executor_workspace_root/"continuity-v6"/trial["execution_id"],task)
