    ...
    async def infer(self, request):
        ...
        if "simulated_provider_capacity_before_request" in self.injections and not self.ledger.injection_consumed("simulated_provider_capacity_before_request"):
            ...
            raise InferenceResourceUnavailable(
                "benchmark injected capacity before send",
                retryable=True,
                request_sent=False,
            )
        if "quota_after_useful_edit" in self.injections and self.useful_edit_observed and not self.ledger.injection_consumed("quota_after_useful_edit"):
            ...
            raise InferenceResourceUnavailable(
                "benchmark injected quota after useful edit",
                retryable=False,
                request_sent=False,
            )
...
def admission_for(runtime_service: RuntimeService, task: dict, root: Path, repos: dict[str, Path], model: str, permitted: tuple[str, ...], *, prior_attempt=None, attempt_override=None, session_id=None, revision=1, contract_additions: dict | None = None):
    ...
    workspace = WorkspaceManifest(workspace_id=uuid4(), work_id=work_id, pwu_id=pwu_id, attempt_id=attempt.id, source_vector_digest=vector.digest or "", host_storage_id=str(root), environment_profile_digest=canonical_digest({"profile":"continuity-v7"}), mounts=tuple(mounts), evidence_namespace=f"continuity-v7:{attempt.id}", retention_policy="benchmark-retained")
    ...
    envelope = ResourceEnvelope(envelope_id=uuid4(), policy_version="continuity-v7", max_inference_submissions=30, max_tool_effects=100, max_active_seconds=1800, max_cost_units=10000, provider_profile=exact_profile(model), permitted_provider_profiles=permitted)
    admitted_paths = [
        workspace_path(mount_id, path)
        for mount_id, repo in task["repositories"].items()
        for path in repo["write_scope"]
    ...
    with database.unit_of_work() as uow:
        rows=uow.session.execute(text("""
            ...
            WHERE u.resource_type='inference_submission'
              ...
              AND COALESCE((s.result_payload->>'request_sent')::boolean,true)
...
def recover_interrupted_current_trial(ledger: BudgetLedger, database) -> None:
    ...
    if failed_unknown_requests and state.runtime_mode is not ExecutionMode.FINISHED:
        ledger.event(
            "PROVIDER_RESPONSE_UNKNOWN_FRONTIER_RETAINED",
            attempt_id=str(attempt_id),
...
async def run_trial(spec: dict, trial: dict, settings: Settings, ledger: BudgetLedger, database) -> dict:
    ...
    if progress["attempts"]:
        ...
        root,repos=prepare_repositories(settings.native_executor_workspace_root/"continuity-v7"/trial["execution_id"],task)
    ...
    for cycle in range(len(segments),30):
        ...
        if state.runtime_mode is ExecutionMode.WAITING_RESOURCE:
            ...
            response_unknown = any(
                item.get("attempt_id") == str(attempt.id)
                and any(
                    pending.get("local_request_id") == item.get("local_request_id")
                    for pending in ledger.data.get("pending_provider_requests", [])
                )
            ...
            unknown_successor_reason = "successor after response-unknown no-effect frontier"
            if response_unknown and not any(
                item["reason"] == unknown_successor_reason
                for item in progress["attempts"]
            ):
                make_successor(
                    model,
                    unknown_successor_reason,
                    additions={
                        "recovery_note": (
                            "A prior Provider request has an unknown response and was not "
                            "replayed. No Tool effect began from that request. Inspect the "
                            "retained workspace and receipts, then advance only the residual "
                            "obligations with a new decision."
                        )
                    },
                )
                worker,offer=make_worker(model,attempt,injections)
            ...
            if response_unknown:
                stop_attempt(
                    runtime,
                    attempt.id,
                    "second response-unknown boundary exceeds bounded successor recovery",
                )
                break
