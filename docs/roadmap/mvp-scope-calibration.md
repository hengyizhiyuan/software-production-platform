...
| [Duration-aware PWU sizing / capacity-aware production planning](../architecture/duration-capacity-semantic-foundation.md) | DCP-1 semantic foundation DEFINED / ADMITTED; runtime behavior not implemented | Current bounded PWU/Attempt loop is proven | Human and Planner use present fit judgment; no autonomous duration decision | Measurement, uncertainty-aware estimation, sizing advisory, and future capacity ownership are separated without weakening Authority | Separately admitted measurement/design/implementation slice |
...
13. **Duration/capacity semantic foundation** — DCP-1 DEFINED / ADMITTED;
    measurement schema, estimation, queue, scheduling, and runtime capacity
    behavior require separate admission and do not reopen Core.
